...
# Species of interest
myspecies <- "Acrocephalus scirpaceus"
...
g1 <- ggplot(data = Europe) +
  ...
  labs(title = "GBIF occurrences of Acrocephalus scirpaceus")+
  xlab("Longitude")+
  ylab("Latitude")
...
g2 <- ggplot(data = Europe) +
  ...
  theme_classic() + xlim(xmin,xmax) + ylim(ymin,ymax) + 
  labs(title = "iNaturalist occurrences of Acrocephalus scirpaceus")
print(g2)
###############################################################################
...
inat_clean <- inat_raw %>%
  ...
  filter(grepl("^Acrocephalus scirpaceus( |$)", scientific_name)) # I keep the subspecies 
...
matrix_full_date_sp3 <- matrix_full %>%
  filter(!is.na(date_obs)) %>%
  filter(date_obs >= date_start & date_obs <= date_end)

# Check results
head(matrix_full_date_sp3)
summary(matrix_full_date_sp3$date_obs)
table(matrix_full_date_sp3$source)

###############################################################################
# 10) MAP OF COMBINED DATA
###############################################################################
quartz()
l3 <- ggplot(data = Europe) +
  geom_sf(fill = "grey95", color = "black") +
  geom_point(
    data = matrix_full_date_sp3,
