...
g2 <- ggplot(data = Europe) +
  ...
  theme_classic() + xlim(xmin,xmax) + ylim(ymin,ymax) + 
  labs(title = "iNaturalist occurrences of Prunella modularis")
print(g2)
###############################################################################
...
matrix_full_date_sp2 <- matrix_full %>%
  filter(!is.na(date_obs)) %>%
  filter(date_obs >= date_start & date_obs <= date_end)

# Check results
head(matrix_full_date_sp2)
summary(matrix_full_date_sp2$date_obs)
table(matrix_full_date_sp2$source)

###############################################################################
# 10) MAP OF COMBINED DATA
###############################################################################
quartz()
f3 <- ggplot(data = Europe) +
  geom_sf(fill = "grey95", color = "black") +
  geom_point(
    data = matrix_full_date_sp2,
  ...
  theme_classic() + xlim(xmin,xmax) + ylim(ymin,ymax)+
  labs(title = "Occurrences of Prunella modularis in Europe")
print(f3)
